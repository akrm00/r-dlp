...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        // Elevation is real: the card sits above the page surface and carries a
        // shadow in light mode, a lit top edge in dark mode.
        "bg-card text-card-foreground border-hairline floating flex flex-col gap-6 rounded-2xl border py-5",
        className
      )}
      {...props}
    />
